...
const returnOneObject = async file => {
  try {
    ...
    global.konsole.log(`error with ${file}: ${error.message}`);
    return null;
  }
};
  ...
  for await (const file of files) {
    ...
    if (fileObj === null) {
      ...
      if (totalNumberOfFailures > 50) {
        global.konsole.log('WEVE FUCKED UP TOO MUCH');
        process.exit(1);
      }
    } else {
      if (onEmitFile !== null) {
      ...
      if (i % logFrequency === 0) {
        global.konsole.log(`done with ${i}/${files.length} files`);
      }
    }
  }
...
async function crawlOneDir(dir, crawlOptions=defaultCrawlOptions, showDirs=false, onEmitFile = null) {
  ...
  global.konsole.log(`done crawling ${dir}. found ${files.length} files`);
  global.konsole.log(`reading files metadata`);
